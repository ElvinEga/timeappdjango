...
__all__ = ['login', 'create_user', 'view_users', 'approve_users', 'delete_user',
           'check_activation_key', 'update_account', 'search_users', 'view_admins']
...
def search_users(request, name):
    ...
    account_name = name
    users = User.objects.filter(Q(first_name__icontains=account_name) | Q(last_name__icontains=account_name)).exclude(
        id=request.user.id)
    if not users:
        return Response([])
